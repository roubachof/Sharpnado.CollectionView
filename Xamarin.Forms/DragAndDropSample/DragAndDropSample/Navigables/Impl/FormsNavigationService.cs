...
namespace DragAndDropSample.Navigables.Impl
{
    ...
    public class FormsNavigationService : INavigationService
    {
        ...
        public async Task<ContentPage> NavigateBackAsync(object parameter = null)
        {
            var page = (ContentPage)await NavigationPage.PopAsync();
            return page;
        }

        public Task DisplayAlert(string title, string message, string cancel)
        {
            return NavigationPage.DisplayAlert(title, message, cancel);
        }
    }
}